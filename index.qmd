...
format:
  revealjs:
    ...
    footer: "phd-second-year.janithwanni.com/"
...
* Official support for
  ...
  * A proxy was created to communicate with an existing rendered detourr widget.
    * Now supporting Shiny namespaces 
...
More documentation changes to be done in the future!
Stay tuned at `casperhart/detourr`
:::
::::
...
## Being agnostic is fun and all, {.center}
...
ggplot(
  ...
  aes(y = fct_rev(as.factor(neuron)), x = val, color = metric, tooltip = seed)
) +
...
<!--     ) -->
<!-- ) + -->
<!--   aes(y = val, x = seed, color = as.factor(neuron), tooltip = seed) + -->
<!--   geom_point(size = 0.25) + -->
<!--   facet_wrap(~metric) + -->
<!--   theme_minimal() + -->
<!--   labs(x = "Seed", y = "F1 Score", color = "Neuron") -->
<!-- ``` -->
...
Visit [random-nn-playground.janithwanni.com](http://random-nn-playground.janithwanni.com)
...
<iframe src="https://ourworldindata.org/grapher/exponential-growth-of-parameters-in-notable-ai-systems?yScale=linear&time=2020-02-13..latest&country=&tab=chart" loading="lazy" style="width: 100%; height: 550px; border: 0px none;" allow="web-share; clipboard-write"></iframe>
...
:::{style = "width: 65vw; margin: 0 auto; display: block"}
...
ganttrify::ganttrify(
  ...
  size_text_relative = 2.5,
  month_breaks = 6,
  colour_palette = c("#E57F12", "#38535E", "#C19C0B")
)
```
